...
Focused checks: `tests/unit/test_execution_controller_routes.py`, `test_capture_stop_recovery_component.py`, `test_rdbg_arbiter.py`, `test_cell_execution_pipeline.py`, `test_route_cell_policies.py`, `test_worker_mutation_runner.py`, `test_worker_module_lifecycle_service.py`, `test_runtime_rdbg_single_owner.py`, `test_session_default_execution_owner.py`, plus legacy `test_runtime_api.py` and `test_prototype_runtime.py`. Static tests do not qualify live 1C behavior; final acceptance must run the notebook against a temporary infobase.
