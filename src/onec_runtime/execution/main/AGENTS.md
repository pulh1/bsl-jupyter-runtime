...
The default public `RuntimeSession` runs MAIN through the arbiter worker and retains one operation across CAPTURE stops. The old `PrototypeRuntimeController` remains only for isolated legacy regression tests; do not attach it to the same RDBG session.
