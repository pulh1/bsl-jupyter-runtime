...
- `idle_materialization.py` runs direct and dynamic Context value/table transfer plus bounded `head_to_df` while MAIN is at a confirmed service stop. Its admission, payload read and private-key deletion use one arbiter ticket and the same SessionPort; the exact idle target and frozen Worker catalog are rechecked before the first RDBG effect. Keep confirmed private-key deletion debts retryable without repeating the parent transfer.
- `completion.py` reads `ЗавершеннаяКоманда`, `Результат`, `Ошибка` and optional messages through the exact pending evaluation capability. Matching command ID marks `MainOperation` remotely completed before decoding result or messages. The new component controller uses this reader; the transitional public controller's `_complete_main()` has not yet switched to it.
- This package does not import concrete controllers or `RdbgSession`. Protocol value models may be shared.

The transitional public controller still exposes `OperationHandle` and `OperationState` for existing clients and invokes the executor on its caller thread. The new component controller instead runs MAIN through the arbiter worker and retains one operation across CAPTURE stops. RuntimeSession/public API wiring, Worker generation and status integration remain migration steps.

Focused checks: `tests/unit/test_main_operation.py`, `test_main_executor.py`, `test_main_completion_reader.py`, `test_execution_controller_routes.py`, `test_capture_stop_recovery_component.py` and public `test_prototype_runtime.py`.
