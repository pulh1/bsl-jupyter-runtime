...
def article_overview_cells():
    """Reader overview adapted from the article notebook, without local state."""
    return [
        md(r'''# BSL в Jupyter: от первой ячейки до живого проведения ЗУП
...
Сообщить(ПроцентПовышения);'''),
        md(r'''## В контексте остаются и методы
...
ПроцентПовышения = 20;'''),
        py(r'''%%bsl
Сообщить(УвеличитьНаПроцент(100000));'''),
        md(r'''Первый вызов выводит 110000, второй — 120000. Функцию мы не переопределяли: изменилось только значение `ПроцентПовышения` в общем notebook-контексте.
...
На этом простом примере видна ключевая модель работы runtime: последовательность ячеек образует один интерактивный эксперимент, а не набор независимых запусков.'''),
        md(r'''## Та же сессия — внутри настоящей ЗУП
...
КадровыеДанные = КадровыйУчет.СотрудникиОрганизации(
    Истина, ПараметрыСотрудников);'''),
        md(r'''## Из 1С в Python'''),
        py(r'''# КадровыеДанные пока живёт в 1С; to_df() переносит таблицу в pandas.
df = КадровыеДанные.to_df(refs="presentation")
display(df.head(10))
print("Строк с кадровыми данными:", len(df))'''),
        md(r'''Для ссылочных значений можно выбрать представление, UUID или оба значения. В обзорном сценарии представления удобны для чтения, а UUID пригодятся, когда нужно устойчиво сравнивать и соединять данные.'''),
        py(r'''df_uuid = КадровыеДанные.to_df(refs="uuid")
...
display(df_both[["Сотрудник", "Сотрудник__uuid", "Подразделение"]].head())'''),
        md(r'''## 1С получает данные, Python исследует их
...
Сгруппируем ФОТ по подразделению и построим диаграмму обычными средствами pandas/matplotlib. Важна граница ответственности: данные и расчёт пришли из ЗУП, Python используется как интерактивный аналитический слой.'''),
        py(r'''import matplotlib.pyplot as plt
...
display(fig)
plt.close(fig)'''),
        md(r'''## Зафиксируем результат до экспериментов
...
ПлановыйФот = ПлановыеНачисленияСотрудников.ТекущиеДанныеОплатыТрудаСотрудников(
    Неопределено, КадровыеДанные);'''),
        py(r'''display(ПлановыйФот.head(10).to_df(refs="presentation"))'''),
        md(r'''## Hot reload: короткий опыт
...
Загрузим изменённый модуль в текущий сеанс и повторим тот же вызов. Механизм hot reload, его границы и сценарии использования подробно разбираются в отдельной статье.'''),
        py(r'''RELOAD_MODULE_PATH = r'CommonModules\ПлановыеНачисленияСотрудников\Ext\Module.bsl'
runtime.load_worker_module(RELOAD_MODULE_PATH)'''),
        py(r'''%%bsl
ПлановыйФотСоСтраховыми = ПлановыеНачисленияСотрудников.ТекущиеДанныеОплатыТрудаСотрудников(
    Неопределено, КадровыеДанные);'''),
        py(r'''display(ПлановыйФотСоСтраховыми.to_df(refs="presentation"))'''),
        md(r'''Изменённый метод нужен здесь только как демонстрация возможности. Теперь перейдём ко второй границе runtime: данным, которые существуют лишь внутри уже выполняющегося типового кода.'''),
        md(r'''## Capture: остановка внутри проведения «Приема на работу»
...
Прием.Записать(РежимЗаписиДокумента.Проведение);'''),
        py(r'''capture_status = runtime.status()
capture_status.state.value'''),
        py(r'''runtime.current_capture().stack[:3].with_methods()'''),
        py(r'''%%bsl
...
ПоказатьОклад(Прием.Ссылка);'''),
        md(r'''Проведение ещё не завершилось. Runtime удерживает тот же вызов, а notebook может исследовать его текущий контекст.'''),
        py(r'''%%bsl
...
Сообщить(Строка(КонтекстОтладки.СтруктураДанных.ЗначенияПоказателей[0].Показатель) +
    " :" + КонтекстОтладки.СтруктураДанных.ЗначенияПоказателей[0].Значение);'''),
        py(r'''completed = runtime.resume_capture()
runtime.clear_capture_points()
print('Продолжилось и завершилось то же проведение')'''),
        md(r'''## Продолжаем тот же вызов и проверяем результат
...
ПланПослеПроведения = ПлановыеНачисленияСотрудников.ТекущиеДанныеОплатыТрудаСотрудников(
    ...
    .Скопировать(Новый Структура("Сотрудник", СотрудникДляОпыта), "Сотрудник,ФОТ, СовокупнаяТарифнаяСтавка, ВидТарифнойСтавки");'''),
        py(r'''ПланПослеПроведения.to_df()'''),
        py(r'''runtime.close()'''),
    ]


def article_ut_sales_cells():
    """Reader UT walkthrough adapted from the article notebook."""
    return [
        md(r'''# Продажи УТ: исследуем данные и типовую логику интерактивно
...
НачалоПредыдущегоМесяца = ДобавитьМесяц(НачалоПоследнегоМесяца, -1);
КонецПоследнегоМесяца = КонецМесяца(ПоследняяПродажа.Период);'''),
        md(r'''## Получаем обороты средствами УТ
...
display(fig)
plt.close(fig)'''),
        py(r'''latest_month = sales['Месяц'].max()
by_department = (sales.loc[sales['Месяц'] == latest_month]
                 .assign(Подразделение=lambda data:
                         data['Подразделение'].fillna('Без подразделения'))
                 .groupby('Подразделение')['Выручка'].sum()
...
display(fig)
plt.close(fig)'''),
        md(r'''## Вызываем типовую бизнес-логику
...
ПриростВыручки = ПродажиСервер.ПроцентПрироста(
    ВыручкаПредыдущегоМесяца, ВыручкаПоследнегоМесяца);
КонтрольныйПриростДо = ПродажиСервер.ПроцентПрироста(0, 100);'''),
        py(r'''growth_before = ПриростВыручки.materialize()
probe_before = КонтрольныйПриростДо.materialize()
print('Прирост выручки, %:', growth_before)
print('Контрольный вызов (0, 100), %:', probe_before)'''),
        md(r'''## Hot reload: проверяем альтернативную трактовку
...
Это лишь короткая демонстрация возможности; устройство hot reload и его ограничения разбираются в отдельном материале.'''),
        py(r'''runtime.load_worker_module(r'CommonModules\ПродажиСервер\Ext\Module.bsl')'''),
        py(r'''%%bsl
...
def build():
    overview = article_overview_cells()
...
print('ФОТ после обычного вызова:', restored['ФОТ'].sum(), '₽')'''),
    ...
    for folder, name, cells in [('ZUP', '01-overview', overview),
                                ...
                                ('UT', '05-ut-sales', article_ut_sales_cells())]:
