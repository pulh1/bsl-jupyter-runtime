...
ЗапросДемоВТ.МенеджерВременныхТаблиц = e1cRuntimeКонтекстОтладки.Запрос.МенеджерВременныхТаблиц;
...
CAPTURE_OUTPUT = '''СнимокВыхода = e1cRuntimeКонтекстОтладки.КадровыеДанныеСотрудников.Скопировать(
...
OVERVIEW_CAPTURE_MUTATE = '''СтрокиОклада = e1cRuntimeКонтекстОтладки.СтруктураДанных.ЗначенияПоказателей.НайтиСтроки(
...
СтрокиФОТ = e1cRuntimeКонтекстОтладки.СтруктураДанных.ДанныеОПлановыхНачислениях.НайтиСтроки(
...
СтрокаСнимка.ВыручкаПредыдущегоМесяца = e1cRuntimeКонтекстОтладки.ПредыдущееЗначение;
СтрокаСнимка.ВыручкаПоследнегоМесяца = e1cRuntimeКонтекстОтладки.ТекущееЗначение;
СтрокаСнимка.ПриростПроцентов = e1cRuntimeКонтекстОтладки.Результат;'''
...
print('Кадров в стеке:', stack['total'])'''),
        md('## Читаем временную таблицу\n\n`e1cRuntimeКонтекстОтладки` доступен, пока исходный вызов остановлен. '
...
def overview_capture_cells():
    return [
        ...
        md('Выполнение документа ещё не завершилось. '
           'Через `e1cRuntimeКонтекстОтладки` видны таблицы именно этого вызова. '
           'Для просмотра копируем только нужные столбцы.'),
        bsl('СнимокПоказателей = e1cRuntimeКонтекстОтладки.СтруктураДанных.ЗначенияПоказателей.Скопировать(\n'
            '    , "Сотрудник,Показатель,Значение");\n'
            'СнимокПлановыхНачислений = e1cRuntimeКонтекстОтладки.СтруктураДанных.ДанныеОПлановыхНачислениях.Скопировать(\n'
...
display(pd.DataFrame(stack['frames'])[['level', 'module_type', 'line']])'''),
        md('## Значения внутри вызова\n\nПока вызов остановлен, '
           '`e1cRuntimeКонтекстОтладки` даёт аргументы и локальный результат. '
...
runtime.add_capture_point(MODULE_PATH, 129)'''),
        ...
        bsl('e1cRuntimeКонтекстОтладки.Запрос.Текст = "ВЫБРАТЬ Сотрудник, Организация, ФОТ * 1.1 КАК ФОТ ИЗ ВТКадровыеДанныеСотрудников";'),
