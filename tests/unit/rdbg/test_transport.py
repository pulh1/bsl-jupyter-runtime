...
def test_non_ping_read_timeout_remains_a_transport_error() -> None:
    def time_out(request: httpx.Request) -> httpx.Response:
        raise httpx.ReadTimeout("planned request timeout", request=request)

    client = httpx.Client(transport=httpx.MockTransport(time_out))
    transport = RdbgTransport("127.0.0.1", 12345, client=client)

    with pytest.raises(RdbgTransportError, match="planned request timeout"):
