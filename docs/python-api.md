...
Обычный `RuntimeSession.start()` выбирает новый маршрут. Полный `01-overview.ipynb` прошёл на временной копии ЗУП КОРП с расширением 0.1.10 / протоколом 5: все 41 кодовая ячейка выполнилась без ошибок, включая загрузку модуля, CAPTURE и закрытие сеанса. На той же остановке отдельно проверен сценарий «ошибка BSL → чтение кадра и переменных → исправление метода → материализация → продолжение». Следующая таблица описывает подтверждённые контракты и текущие ограничения.

| Возможность | Состояние проверки |
| --- | --- |
| `execute_bsl()`, `status()`, `namespace_snapshot()`, `resume_capture()`, `resume_debug_stop()` | Публичные вызовы связаны с controller/arbiter в компонентных тестах; MAIN, CAPTURE и resume прошли в полном живом прогоне notebook. |
| `current_capture().stack`, `context.variables`, переменные кадров | На временной ИБ проверены стек, страницы `frame.variables`, `frame.locals`, `frame.parameters` и точечное чтение корня. Точечный запрос даёт тип, размер и скрытый preview; раскрытие потомков текущим маршрутом не поддерживается. |
...
Типы `WorkerModuleUnit` и `WorkerGenerationHandle` импортируются из `onec_runtime.bsl.module_universe` и `onec_runtime.worker_universe`, типы точек и отчёта — из `onec_runtime.worker_breakpoints`. Загрузка и изменение точек проходят через ticket одного RDBG arbiter; загрузка модуля и CAPTURE-точка прошли в полном живом прогоне notebook.
...
При использовании `RuntimeSession` без Jupyter те же операции доступны по ограниченному handle постоянного контекста: `runtime.materialize("e1cRuntimeКонтекст.Количество")` и `runtime.to_df("e1cRuntimeКонтекст.Таблица", refs="uuid")`. Handle должен быть прямым или точечным путём от `e1cRuntimeКонтекст`, без вызовов методов и индексов. Эти методы принимают те же параметры политики ссылок; `materialize()` дополнительно принимает `max_depth`, `max_items`, `max_bytes` и `timeout_s`. На новом маршруте `to_df()` ограничен 100 000 строк и 64 МиБ, а `chunk_size` проверяется как положительная подсказка и пока не разбивает перенос на части. С расширением 0.1.10 и протоколом 5 превышение лимита строк даёт ошибку, а не усечённый `DataFrame`.
...
`StackPage` содержит `frames`, `total`, `next_cursor`; `with_methods()` добавляет разрешённые по исходникам имена и сигнатуры методов. `DebugFrame` содержит `source`, `line`, `native_level`, `source_status`, `method_status`; `frame.variables`, `frame.parameters`, `frame.locals` открывают переменные этого кадра. Страницы переменных содержат `items`, `total`, `next_cursor` и проверенные метаданные каждого доступного значения. `ValueNode` из страницы или точечного запроса показывает `name`, `type_name`, скрытый `preview`, `size`, `shape`, `expandable`. `frame.parameters` и `frame.locals` разделяют имена по сигнатуре метода, если его исходник доступен. Страница `capture.context.variables` пока перечисляет только имена; для типа и размера используйте точечный запрос. В текущем публичном маршруте раскрытие через `children`, `fields`, `items`, `columns`, `rows` вызывает `CaptureShapeUnsupportedError`; для содержимого используйте ограниченную материализацию. Выбор страницы требует конечного среза; бесконечная итерация намеренно недоступна.
