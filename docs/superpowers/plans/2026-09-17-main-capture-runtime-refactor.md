...
- [ ] Pure preparation produces Worker candidate intent, namespace/message metadata and guarded snapshots; accepted ticket owns Worker activation/pin and publication. The intent retains the complete method projection and versioned prior method set so omitted methods survive redefinition. Every MAIN command using an existing Worker generation also owns a pin through CAPTURE and resume; each CAPTURE cell owns its own pin and updates the stopped Worker slot when the active generation changes. Method-only cells must work. A failed or ambiguous pin release is recorded as a typed owner debt after preserving the confirmed MAIN/CAPTURE outcome. Queue release as a dependent arbiter ticket after route handoff but before later user dispatch: the user ticket settles with its confirmed reply, while the cleanup ticket owns any uncertain RDBG capability and fences dependent Worker work. Repair or quarantine the debt without replacing the cell result. The real Worker mutation executor must use the supplied `SessionPort`, never the legacy `RuntimeApi` callback.
- [ ] MAIN/CAPTURE settlement returns the existing `RuntimeReply`/provenance and proxy generations through route-specific policy services. Retain MAIN publication context (source maps, message key, namespace candidate, Worker pin and capture correlation) across CAPTURE stops so a later resume can publish the same command's terminal reply. Collect remote messages on the arbiter worker before policy settlement. A confirmed CAPTURE BSL/decode error becomes a failed cell reply without losing the scope; publish namespace only after successful settlement. Map the public stop sequence per MAIN command independently of the component's local fence. `RuntimeSession.execute_bsl()` releases its operation lock before ticket waiting.
...
- [ ] KeyboardInterrupt/Stop completes the fenced ticket's target teardown or reports `stop_unknown`; confirmed replacement closes old core admission and detaches the Jupyter `bsl` bridge before exposing the new runtime. Saved value and frame handles must be stale immediately after Stop, including the interval before a replacement is installed.
