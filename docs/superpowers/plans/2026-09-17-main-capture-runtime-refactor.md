...
- [ ] Pure preparation produces Worker candidate intent, namespace/message metadata and guarded snapshots; accepted ticket owns Worker activation/pin and publication. Method-only cells must work. A failed or ambiguous pin release is recorded as a typed owner debt after preserving the confirmed MAIN/CAPTURE outcome; repair or quarantine runs through the arbiter instead of replacing that outcome with a cleanup exception.
