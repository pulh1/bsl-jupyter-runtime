...
Дата: 17.09.2026. База анализа: master, 3d5db0a. Это согласованная спецификация рефакторинга MAIN/CAPTURE. Исторические упоминания `prototype_runtime.py`/`runtime_api.py` ниже описывают исходный master, а не действующую реализацию.

Состояние реализации на 18.09.2026: публичный `RuntimeSession.start()` использует `PublicExecutionFacade`, `CellExecutionPipeline`, `ExecutionController`, MAIN/CAPTURE executors и один `RdbgArbiter`; старые controller/API/coordinator удалены. Исходный `01-overview.ipynb` полностью прошёл на временной файловой ИБ: 41 исполняемая ячейка без ошибок, исходники 63 ячеек сохранены, кроме настройки подключения. На CAPTURE stop отдельно подтверждены ошибка BSL → чтение кадра и переменных → исправление метода → материализация → продолжение. Настоящий notebook Ctrl+C во время удалённых MAIN и CAPTURE eval подтвердил завершение точного target, замену сеанса и недействительность старых proxy/view; для CAPTURE наблюдался ровно один `evalExpr`. Серверная ИБ этими live результатами не квалифицирована.
...
Для исполнения подготовленной ячейки действует общий контракт. Возможности чтения остановленного кадра, материализации и resume доступны через отдельный контракт CAPTURE. Исходный `CaptureEvaluationCoordinator` заменён общим механизмом ticket/waiter внутри `RdbgArbiter`. `RdbgSession` сохраняет RDBG XML, protocol state и корреляцию `result_id`; HTTP и сетевые ошибки принадлежат `RdbgTransport`. Arbiter не копирует protocol state сессии.
...
Исторические точки кода для этого раздела относятся к `master` на коммите `3d5db0a`: `src/onec_runtime/session.py`, удалённые в данной ветке `src/onec_runtime/runtime_api.py` и `src/onec_runtime/prototype_runtime.py`, а также `src/onec_runtime/rdbg/session.py`.
...
flowchart TB
    ...
    Session --> API[PublicExecutionFacade: публичный фасад]
...
| `PublicExecutionFacade` | Один runtime | Публичный фасад BSL/status/value APIs; передаёт BSL-запрос в pipeline. Получает Worker/namespace сервисы от composition root; не выбирает MAIN/CAPTURE для lowering или RDBG dispatch. |
...
| CAPTURE | `CaptureExecutor`: подготовить остановленный кадр; построить вызов `RuntimeKernelServer.ВыполнитьКодВКонтекстеОтладки(e1cRuntimeКонтекст, Код)`; отправить `evalExpr`; сопоставить `exprEvaluated`; выполнить inspection/materialization; при resume выгрузить dirty roots, очистить контекст и отправить `Continue`. | `CaptureScope`: frame identity, стадия setup, pending operation, dirty-root journal, ресурсные долги и статус кадра. |
...
`CellPolicy` — узкий route-specific port: `prepare(...) -> PreparedCell | SourceDiagnostic` и `settle(remote_outcome, prepared, services) -> RuntimeReply`. Здесь «общий lowerer» означает **общий семантический обход/переписывание**, а не полное отсутствие различий. В [нынешнем lowerer](../../../src/onec_runtime/bsl/semantic_lowering.py#L500) прямые ветки `LoweringMode` задают имя результирующей переменной, доступ к `e1cRuntimeКонтекстОтладки` и метку source map; запись dirty capture root связана с правилом доступа к этому namespace. Целевой контракт выносит эти решения в immutable `LoweringProfile` выбранной policy: `result_channel`, правило чтения/записи capture namespace с учётом dirty roots и `source_map_tag`. Общий обход AST получает profile, а не проверяет MAIN/CAPTURE enum. Если алгоритмы переписывания впоследствии действительно разойдутся, разделить их на две реализации будет оправданно; сейчас дублировать весь обход ради этих различий не нужно.
...
`MainOperation` «живёт» как запись о незавершённой **команде 1С**, а не как всё ещё выполняющийся Python-вызов notebook. При CAPTURE stop исходный запрос MAIN-ячейки уже может вернуть `CaptureView`, но стек 1С остаётся остановлен внутри `Выполнить(ТекущаяИнструкция)`. Ядро запишет `ЗавершеннаяКоманда = ИдентификаторКоманды` только после возврата из этого вызова. Поэтому после resume ожидается completion с тем же command ID. В реализации этому соответствуют [цикл 1С](../../../onec/OnecInteractiveRuntime/CommonModules/RuntimeKernelServer/Ext/Module.bsl#L118), `execution/main/operation.py` и проверка completion ID в `execution/main/completion.py`; в исходном master эти действия были сосредоточены в `prototype_runtime.py`.
...
| `context` | `opening`, `ready`, `setup_failed`, `closing`, `closed` | Создан ли и доступен ли `e1cRuntimeКонтекстОтладки`; ошибка setup не меняет `frame_identity` без отдельного доказательства. |
...
Для подтверждённого исхода операции есть один порядок публикации: arbiter worker записывает её результат в публичный CAPTURE ledger **до** settlement ticket и пробуждения синхронно ожидающей notebook-ячейки. Поэтому после возврата или подтверждённой ошибки `ticket.wait()` текущий `CaptureStatus` уже отражает завершённую операцию и допускает следующий запрос, если нет отдельного ресурсного долга. Observer может перенести в ledger неизвестный исход или отмену до эффекта, но его планирование другим потоком не должно задерживать публикацию подтверждённой ошибки `to_df()`, CAPTURE BSL, completion helper или локального admission отказа.
...
Это изменение поведения обеих веток относительно master `3d5db0a`: прежний `_execute_main` в `prototype_runtime.py` ловил `BaseException`, включая `KeyboardInterrupt`, и при отсутствии распознанного stop мог перевести контроллер в `FAILED`; прежний CAPTURE в `capture_evaluation.py` отсоединял waiter и оставлял принятую операцию своему worker. Новый arbiter сохраняет owner pending результата, но дополнительно обрабатывает Stop как намерение прекратить удалённую работу. Внутренний detach всё ещё нужен при утрате вызывающего waiter, однако не является реализацией пользовательской кнопки Stop.
...
`CaptureExecutor.open(scope)` обновляет журнал стадий в `CaptureScope`: stop принят, locals прочитаны, структура перенесена во временное хранилище, kernel frame найден, ID MAIN сверен, `e1cRuntimeКонтекстОтладки` создан. Каждая стадия имеет подтверждённый/неизвестный исход и свои cleanup leases. Повтор возможен только с учётом уже выполненных стадий и подтверждённой прежней остановки. Ошибка чтения locals либо поиска frame завершает setup-запрос, но не равна потере target.

Пользовательский CAPTURE BSL выполняется через `evalExpr(RuntimeKernelServer.ВыполнитьКодВКонтекстеОтладки(e1cRuntimeКонтекст, Код))`. Понижение отмечает прямые присваивания `e1cRuntimeКонтекстОтладки.Имя = ...` как dirty roots. После подтверждённого исполнения — в том числе результата BSL с ошибкой — root names учитываются в ledger. Если исход eval неизвестен, ledger помечается неопределённым и дальнейший resume закрыт до выяснения. Изменение содержимого изменяемого объекта по ссылке может повлиять на исходный кадр уже во время eval; эта семантика не объявляется отложенным writeback.
...
1. Отдельный `evalExpr` вызывает `ПоместитьЗначениеКонтекстаОтладки(e1cRuntimeКонтекст, Имя)` и возвращает адрес temporary storage.
...
После всех roots отдельный `evalExpr` удаляет `e1cRuntimeКонтекстОтладки`, затем отдельный `step/Continue` запускает MAIN. Подтверждённая ошибка выгрузки первого root до первой записи оставляет кадр paused и позволяет исправить причину. После частичной либо неизвестной записи повтор полного resume запрещён: восстановление работает с точным root journal. Подтверждённый `Continue` немедленно инвалидирует старые CaptureView/value handles. При неизвестном исходе `Continue` проверяются target и MAIN identity; ошибку нельзя автоматически трактовать ни как потерю кадра, ни как всё ещё доступный старый кадр.

Для следующей CAPTURE-точки локальный successor admission заранее связывает intent с **той же** `MainOperation`, текущим остановленным `CaptureScope` и ожидаемым следующим stop sequence. Это ещё не `commit`: сначала `resume()` устанавливает successor breakpoints, выполняет writeback и Continue, затем сверяет следующий stop или терминальный исход MAIN. Только после подтверждённого Continue и корреляции результата вызывается `admission.commit()`. Если до `modifyValue` и Continue подтверждённо отказала выгрузка root, `admission.rollback()` восстанавливает прежний breakpoint workspace через arbiter, сохраняет точное evidence ошибки, снимает заморозку writeback и возвращает прежний CAPTURE ticket; dirty-root names остаются учтёнными. Подтверждённые записи root, неизвестный RDBG исход или попытка Continue запрещают такой rollback по одному исключению.
...
У каждой CAPTURE-ячейки есть собственная запись прогресса, отличная от `CaptureScope` и arbiter ticket. Она сохраняет подтверждённый результат `evalExpr` **до** восстановления breakpoint workspace, а затем отмечает восстановление, policy и cleanup. Если восстановление отказало до транспортной отправки, тот же ticket может продолжить с сохранённым результатом без второго выполнения BSL. После успешного ремонта подтверждённая ошибка самой ячейки завершает ticket как ошибка и освобождает очередь; она не переводит scope в `lost`. Неизвестный исход уже отправленного восстановления требует отдельной сверки и не допускает слепого повтора.
...
`RuntimeSession` создаёт `RdbgSession` и после bootstrap передаёт владение им arbiter; контроллеру передаются arbiter port, workspace owner и готовые MAIN/CAPTURE route bindings. Контроллер создаёт только объекты состояния на явном переходе: новую `MainOperation` при MAIN-команде или новый `CaptureScope` при распознанном capture stop. Замена способа исполнения **существующего** режима может выбрать другую реализацию executor при том же contract. Новый режим требует ещё правила входа/выхода, lowering profile, набора capabilities и outcome/status adapter; один новый Python-класс этого не обеспечивает. При двух текущих режимах явные transition rules проверяемее общего динамического plugin registry.

Composition root также связывает `MainRoute = (MainCellPolicy, MainExecutor)` и `CaptureRoute = (CaptureCellPolicy, CaptureExecutor)` с контроллером, затем создаёт один `CellExecutionPipeline` из common parser, controller port и namespace/Worker service ports. `PublicExecutionFacade` получает pipeline как зависимость. Только controller знает, какой route сейчас выбран; pipeline получает его `PreparationContext` и не собирает route из enum или имени класса.

Реализация использует `ExecutionController`, `ExecutionControllerPort` и `PublicExecutionFacade`. По решению о прекращении совместимости старые `prototype_runtime.py`, `runtime_api.py` и `CaptureEvaluationCoordinator` удалены; публичные DTO вынесены в `runtime_models.py`, `capture_evaluation_models.py`, `capture_transfer_models.py` и `execution/continuation_models.py`. Тесты старой формы заменены проверками публичного маршрута и самостоятельных компонентов. Из `ExecutionController` уже вынесены Stop observation и часть CAPTURE data plane. Локальное планирование continuation и Worker lease accounting остаются задачами структурного упрощения без второго владельца RDBG.
...
6. Свести controller к маршрутизации готовых исполнителей и созданию MainOperation/CaptureScope, переименовать класс, организовать каталоги и содержательные `AGENTS.md`. Общие публичные типы перенести к их текущему владельцу без экспорта удалённых классов для совместимости. Удалить mode-specific `getattr` и ветвление исполнения по `controller.state` из `RuntimeApi`.
...
Минимальный focused набор действующего маршрута: `test_execution_controller_routes.py`, `test_direct_ticket_interrupt_stop.py`, `test_main_dispatch_ticket_evidence.py`, `test_capture_ticket_data_plane.py`, `test_runtime_rdbg_single_owner.py`, `test_jupyter_confirmed_stop_replacement.py` и `tests/unit/rdbg/test_session.py`. Для issue #7 отдельно нужен live сценарий: BSL-ошибка → исправление и повтор в том же CAPTURE → `to_df()` → конкретный доступ к `variables` → resume. Он требует установленной 1С и не входит в статическую квалификацию.
