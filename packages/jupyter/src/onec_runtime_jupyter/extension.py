...
def _display_reply(
    ...
    *,
    ...
    if selected.mode == "presentation" and not reply.succeeded:
        ...
        # A native frame has its own 1C module coordinate. The callback proves
        # the current request, but its prepared map may differ from the final
        # controller wrapper retained by the diagnostic.
...
def _diagnostic_source_root(
    runtime: object, reply_kind: RuntimeReplyKind
) -> Path | str | None:
    try:
